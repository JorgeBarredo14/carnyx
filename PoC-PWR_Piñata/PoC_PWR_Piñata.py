...
n_fuzz = 1
device = "pinata"
date = "2023_11_22_11o10"  # Example date, adjust as needed

# Get the directory of the current script
script_dir = os.path.dirname(os.path.abspath(__file__))
# Output zip filename to combine parts
zip_filename = os.path.join(script_dir, 'results/' + device + '_PWR_bugs_' + date + '.zip')
...
if os.path.isfile(zip_filename):
    ...
    extract_to = os.path.join(script_dir, 'results/' + device + '_PWR_bugs_' + date)
    ...
    base_path = './results/' + device + '_PWR_bugs_' + date + '/'
    list_nfs_path = base_path # Anonymized path
